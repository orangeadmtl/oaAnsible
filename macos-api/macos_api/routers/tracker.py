...
async def get_tracker_stats():
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"Error connecting to oaTracker API: {str(e)}")
...
async def get_tracker_status():
    ...
    try:
        ...
        return {
            ...
            "api": api_status,
            "healthy": service_status.get("healthy", False) and api_status["api_accessible"],
            "status": "online" if (service_status.get("healthy", False) and api_status["api_accessible"]) else "error"
        }
    except Exception as e:
        logger.error(f"Error getting tracker status: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error getting tracker status: {str(e)}")
...
async def get_tracker_stream():
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"Error connecting to oaTracker stream: {str(e)}")
    except Exception as e:
...
async def get_tracker_mjpeg_stream():
    ...
    try:
        ...
        async def stream_generator():
            async with httpx.AsyncClient() as client:
                async with client.stream("GET", f"{TRACKER_API_URL}/mjpeg", timeout=None) as response:
        ...
        raise HTTPException(status_code=500, detail=f"Error streaming from oaTracker: {str(e)}")
