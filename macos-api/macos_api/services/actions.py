    ...
    async def restart_tracker(self) -> str:
        ...
        try:
            ...
            if unload_process.returncode != 0:
                error_msg = unload_stderr.decode().strip() if unload_stderr else "Unknown error"
                logger.warning(f"Tracker unload warning (might be normal if not running): {error_msg}")
            ...
            if load_process.returncode != 0:
                error_msg = load_stderr.decode().strip() if load_stderr else "Unknown error"
