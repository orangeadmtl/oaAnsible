    ...
    try:
        ...
        if enabled:
            try:
                ...
                progress_match = re.search(r"Encryption Progress:\s+(\d+\.\d+)%", progress_output)
                if progress_match:
                    details["progress"] = float(progress_match.group(1))
                
                # Check for recovery key information
                recovery_output = run_command(["fdesetup", "hasinstitutionalrecoverykey"])
                details["has_institutional_recovery"] = "true" in recovery_output.lower()
                
                personal_recovery_output = run_command(["fdesetup", "haspersonalrecoverykey"])
                details["has_personal_recovery"] = "true" in personal_recovery_output.lower()
...
def check_gatekeeper_status() -> Dict:
    ...
    try:
        ...
        try:
            ...
            notarization_output = run_command(["defaults", "read", "/Library/Preferences/com.apple.security", "GKAutoUpdateEnabled"])
...
def check_sip_status() -> Dict:
    ...
    try:
        ...
        if "Filesystem Protections" in output:
            details["filesystem_protections"] = "enabled" in output.split("Filesystem Protections:")[1].split("\n")[0].lower()
        if "Debugging Restrictions" in output:
            details["debugging_restrictions"] = "enabled" in output.split("Debugging Restrictions:")[1].split("\n")[0].lower()
        if "DTrace Restrictions" in output:
            details["dtrace_restrictions"] = "enabled" in output.split("DTrace Restrictions:")[1].split("\n")[0].lower()
        if "NVRAM Protections" in output:
            details["nvram_protections"] = "enabled" in output.split("NVRAM Protections:")[1].split("\n")[0].lower()
...
def check_firewall_status() -> Dict:
    ...
    try:
        ...
        output = run_command(["/usr/libexec/ApplicationFirewall/socketfilterfw", "--getglobalstate"])
        ...
        try:
            ...
            stealth_output = run_command(["/usr/libexec/ApplicationFirewall/socketfilterfw", "--getstealthmode"])
            details["stealth_mode"] = "enabled" in stealth_output.lower()
            
            # Check if logging is enabled
            logging_output = run_command(["/usr/libexec/ApplicationFirewall/socketfilterfw", "--getloggingmode"])
            details["logging"] = "enabled" in logging_output.lower()
            
            # Check if automatic allow signed apps is enabled
            signed_output = run_command(["/usr/libexec/ApplicationFirewall/socketfilterfw", "--getallowsigned"])
            details["allow_signed"] = "ENABLED" in signed_output
            
            # Get list of allowed apps
            try:
                apps_output = run_command(["/usr/libexec/ApplicationFirewall/socketfilterfw", "--listapps"])
                allowed_apps = []
                ...
                details["allowed_apps"] = allowed_apps[:10]  # Limit to 10 apps to avoid excessive data
    ...
    try:
        ...
        try:
            ...
            if os.path.exists("/Library/Receipts/InstallHistory.plist"):
                history_output = run_command(["plutil", "-convert", "json", "-o", "-", "/Library/Receipts/InstallHistory.plist"])
                history = json.loads(history_output)
                
                # Find the most recent security update
                security_installs = [
                    entry for entry in history 
                    if "Security Update" in entry.get("displayName", "") or "security" in entry.get("displayName", "").lower()
                ]
                
                if security_installs:
                    # Sort by date, most recent first
                    security_installs.sort(key=lambda x: x.get("date", ""), reverse=True)
        ...
        update_policy = {}
        try:
            auto_check_output = run_command(["defaults", "read", "/Library/Preferences/com.apple.SoftwareUpdate", "AutomaticCheckEnabled"])
            update_policy["automatic_check"] = "1" in auto_check_output
            
            auto_download_output = run_command(["defaults", "read", "/Library/Preferences/com.apple.SoftwareUpdate", "AutomaticDownload"])
            update_policy["automatic_download"] = "1" in auto_download_output
            
            critical_updates_output = run_command(["defaults", "read", "/Library/Preferences/com.apple.SoftwareUpdate", "CriticalUpdateInstall"])
    ...
    try:
        ...
        if "Secure Boot: Full Security" in output:
            ...
            if match:
                ...
                enabled = "none" not in value.lower() and "no security" not in value.lower()
            else:
                enabled = False
        ...
        return {
            ...
            "status": "enabled" if enabled else "disabled",
            "details": {
                "platform": "Intel with T2",
                "level": secure_boot_level
            },
            "raw_output": output
        }
    except Exception as e:
...
def get_security_overview() -> Dict:
    ...
    if security_status["firewall"]["enabled"]:
        score += 1
    if security_status["secure_boot"]["enabled"] or security_status["secure_boot"]["status"] == "not_supported":
        score += 1
    ...
    if not security_status["filevault"]["enabled"]:
        recommendations.append("Enable FileVault disk encryption to protect data at rest")
    
    if not security_status["gatekeeper"]["enabled"]:
        recommendations.append("Enable Gatekeeper to protect against malicious software")
    
    if not security_status["sip"]["enabled"]:
        recommendations.append("Enable System Integrity Protection (SIP) to protect system files")
    
    if not security_status["firewall"]["enabled"]:
        recommendations.append("Enable the macOS firewall to protect against network threats")
    
    if not security_status["secure_boot"]["enabled"] and security_status["secure_boot"]["status"] != "not_supported":
        recommendations.append("Enable Secure Boot for hardware-level security (if supported)")
    
    if security_status["security_updates"]["security_updates_available"]:
        recommendations.append("Install available security updates to patch vulnerabilities")
