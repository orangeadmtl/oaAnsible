...
class TailscaleSubnetMiddleware(BaseHTTPMiddleware):
    def __init__(
        self, app, tailscale_subnet_str=TAILSCALE_SUBNET
    ):  # Renamed to avoid conflict
        super().__init__(app)
        try:
            # strict=False allows host IPs within the network definition itself
            self.tailscale_subnet = ipaddress.ip_network(
                tailscale_subnet_str, strict=False
            )
            logger.info(
                f"TailscaleSubnetMiddleware initialized with subnet: {self.tailscale_subnet}"
            )
        except ValueError as e:
            logger.error(
                f"Invalid TAILSCALE_SUBNET format: {tailscale_subnet_str}. Error: {e}"
            )
            ...
            logger.warning(
                f"TAILSCALE_SUBNET '{tailscale_subnet_str}' is invalid. Middleware will be extremely restrictive."
            )
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            logger.error(
                f"Invalid client IP format: {client_ip_str} in TailscaleSubnetMiddleware"
            )
            raise HTTPException(
                status_code=400, detail="Invalid client IP address format"
            )
        ...
        if request.method == "POST":
            ...
            if client_ip_obj not in self.tailscale_subnet:
                logger.warning(
                    f"POST Access DENIED for IP {client_ip_str} (not in Tailscale subnet {self.tailscale_subnet})"
                )
                raise HTTPException(
                    status_code=403,
                    detail="Access denied: POST requests must originate from the Tailscale network.",
                )
            logger.debug(
                f"POST Access ALLOWED for IP {client_ip_str} (in Tailscale subnet)"
            )
        else:  # For GET, OPTIONS, HEAD, etc.
            if is_localhost:
                logger.debug(
                    f"{request.method} Access ALLOWED for IP {client_ip_str} (localhost)"
                )
                ...
                raise HTTPException(
                    status_code=403,
                    detail="Access denied: Request must originate from localhost or the Tailscale network.",
                )
            else:
                logger.debug(
                    f"{request.method} Access ALLOWED for IP {client_ip_str} (in Tailscale subnet)"
                )
